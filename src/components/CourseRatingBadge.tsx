...
export const CourseRatingBadge: React.FC<CourseRatingBadgeProps> = ({
  ...
  if (position === 'top-left') {
    posClass = 'absolute top-2.5 left-2.5 z-10';
  } else if (position === 'top-right') {
    posClass = 'absolute top-2.5 right-2.5 z-10';
  } else if (position === 'bottom-left') {
    posClass = 'absolute bottom-2.5 left-2.5 z-10';
  } else if (position === 'bottom-right') {
    posClass = 'absolute bottom-2.5 right-2.5 z-10';
  } else if (position === 'corner-edge') {
    posClass = 'absolute top-2 left-2 z-10';
  }
  ...
  return (
    <Component
      ...
      onClick={onClick}
      className={`inline-flex items-center rounded-xl bg-[#0D1B3E]/90 dark:bg-[#0D1B3E]/95 backdrop-blur-md text-[#F5B301] border border-[#F5B301]/40 shadow-md transition-transform hover:scale-105 select-none ${sizeClasses[size]} ${posClass} ${onClick ? 'cursor-pointer' : ''} ${className}`}
