...
interface MascotWithAccessoriesProps {
  gender?: 'male' | 'female';
  ...
  onClick?: () => void;
  showWardrobeHint?: boolean;
  rankStats?: StudentRankStats;
}

export const MascotWithAccessories: React.FC<MascotWithAccessoriesProps> = ({
  gender = 'male',
  isHalfBody = true,
  className = '',
  imageClassName = '',
  ...
  const isFirst = rankStats?.isFirstOnPlatform ?? false;
  const isTopThree = rankStats?.isTopThree ?? false;
  const rank = rankStats?.rank ?? 1;
  const level = rankStats?.level;
  ...
  return (
    ...
    >
      ...
      {/* 2. Top-Rank Floating Indicator on Mascot */}
      {rankStats && (
        <div 
      ...
      {/* 3. Bottom Points Floating Badge */}
      {rankStats && (
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-20 pointer-events-none">
          <div className="flex items-center gap-1 px-2.5 py-0.5 rounded-xl bg-slate-950/80 text-white text-[10px] sm:text-[11px] font-mono font-black border border-white/20 shadow-md backdrop-blur-xs">
            ...
            <span>{rankStats.points} نقطة</span>
          </div>
        </div>
      )}
    </div>
