...
export const FloatingSupportButton: React.FC<FloatingSupportButtonProps> = ({ currentView }) => {
  ...
  const handleSupportClick = () => {
    ...
    const defaultMsg = encodeURIComponent('مرحبًا، أحتاج إلى مساعدة بخصوص حسابي في منصة ويكي فيزياء');
  ...
  return (
    <button
      ...
      className="fixed bottom-24 sm:bottom-8 left-5 sm:left-8 z-40 flex h-13 w-13 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-[#F5B301] text-[#0D1B3E] shadow-xl shadow-[#F5B301]/30 border-2 border-white/90 dark:border-slate-800 transition-all hover:scale-110 active:scale-95 hover:bg-[#e0a401] hover:shadow-2xl group focus:outline-none focus:ring-4 focus:ring-[#F5B301]/40"
    ...
    >
      ...
      <span className="absolute -top-1 -right-1 flex h-4 w-4">
        ...
        <span className="relative inline-flex rounded-full h-4 w-4 bg-emerald-500 border-2 border-white dark:border-slate-800" />
      </span>
    </button>
  );
};
